...
# stack is easily implemented as an Array that just uses the #push and 
# #pop methods.
...
Output: print values to screen

Rules:
- conduct operations as directed by input
- print values to screen when requested

Algorithm:
- create a method for each operation
- create a flow control for processing operations
...
def minilang(instructions)
  stack = []
  register = 0
  instr_array = instructions.split
  instr_array.each do |instruction|
    case instruction
    when 'PUSH'
      push(stack, register)
    when 'ADD'
