...
# to the associated block. The value returned by the block is not used.
# The object that is passed to the block is defined as an argument to
# each_with_object; with each iteration, this object may be updated by the block.
...
# until to iterate through the Array passed in as an argument,
# but must not use any other methods that iterate through an Array
# or any other collection.

=begin

input: array, enumerable object
output: enumerable object

rules:
- iterate through array, yielding the element of the array to the block
- return the enumerable object if array is empty

algorithm:
- iterate through the array with #each
- pass in each element and enumerable object to the block
